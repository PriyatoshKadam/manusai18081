...
export async function POST(req: NextRequest) {
  ...
  const delivered = await notifySlack(webhook, { siteId, severity: 'critical', category: 'configuration_test', vendor: 'gafix', eventName: 'slack_test', message: 'GAfix Slack integration is connected and ready for high-priority incidents.', rootCause: 'This is a customer-requested delivery test, not a production incident.', fixSteps: ['No action required.'] });
