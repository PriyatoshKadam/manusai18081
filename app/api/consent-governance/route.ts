...
export async function GET(req: NextRequest) {
  ...
  const [summaryResult, trendResult, destinationResult, violationResult, firstRequestResult, noSignalResult] = await Promise.all([
    query(`WITH s AS (
  ...
  const currentRate = pct(Number(summary.accepted||0), Number(summary.sessions||0));
  ...
  if (recent.sessions>=20 && prior.sessions>=20 && rateDrop <= -10) issues.push({code:'consent_acceptance_drop',severity:'warning',title:'Consent acceptance rate dropped',message:`Acceptance is ${currentRate}% versus ${priorRate}% in the preceding comparison window (${Math.abs(rateDrop).toFixed(1)} percentage points lower).`,timestamp:trend[trend.length-1]?.day ? new Date(trend[trend.length-1].day).toISOString() : null});
  const currentRejected=pct(Number(summary.rejected||0),Number(summary.sessions||0)); const priorRejected=100-priorRate;
  if (recent.sessions>=20 && prior.sessions>=20 && currentRejected-priorRejected>=10) issues.push({code:'consent_rejection_spike',severity:'warning',title:'Consent rejection rate increased',message:`Rejection is ${currentRejected}% versus approximately ${priorRejected.toFixed(1)}% in the preceding comparison window.`,timestamp:trend[trend.length-1]?.day ? new Date(trend[trend.length-1].day).toISOString() : null});
  const ignoredRate=pct(Number(summary.ignored||0),Number(summary.sessions||0));
  if (ignoredRate>=20 && Number(summary.sessions||0)>=20) issues.push({code:'consent_no_choice_spike',severity:'warning',title:'Consent choices are missing',message:`${ignoredRate}% of observed sessions have no recorded consent choice. Check CMP initialization and consent update timing.`,timestamp:trend[trend.length-1]?.day ? new Date(trend[trend.length-1].day).toISOString() : null});
  const noSignalRate=pct(Number(summary.no_signal||0),Number(summary.sessions||0));
  if (noSignalRate>=20 && Number(summary.sessions||0)>=20) issues.push({code:'consent_never_detected',severity:'warning',title:'Consent signal not detected',message:`${noSignalRate}% of sessions contain no analytics_storage, ad_storage or consent_gcs signal anywhere in the observed session.`,timestamp:trend[trend.length-1]?.day ? new Date(trend[trend.length-1].day).toISOString() : null});
  ...
  return NextResponse.json({siteId,vendor,summary:{sessions:Number(summary.sessions||0),accepted:Number(summary.accepted||0),rejected:Number(summary.rejected||0),partial:Number(summary.partial||0),ignored:Number(summary.ignored||0),signalled:Number(summary.signalled||0),no_signal:Number(summary.no_signal||0),first_request_missing:Number(summary.first_request_missing||0),acceptance_rate:currentRate,change_vs_prior:change(currentRate,priorRate)},trend,destinations,issues,first_request_examples:firstRequestResult.rows.slice(0,50),no_signal_sessions:Number(noSignalResult.rows[0]?.no_signal||0),recent_violations:violationResult.rows.slice(0,100)});
}
