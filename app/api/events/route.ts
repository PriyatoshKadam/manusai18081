...
const failedDelivery = `${networkObservation} AND (delivery_outcome IN ('http_error','blocked','beacon_rejected') OR (${legacyOutcome} AND ((status_code IS NOT NULL AND status_code >= 400) OR failure_reason IN ('blocked','beacon_rejected'))))`;
...
export async function GET(req: NextRequest) {
  ...
  const stats = await query(`SELECT
    ...
    (SELECT CASE WHEN COUNT(*)<30 THEN NULL ELSE ROUND(100.0*COUNT(*) FILTER(WHERE detection_status='scored')/COUNT(*),1) END FROM events WHERE site_id=$1 AND received_at>NOW()-INTERVAL '24 hours') AS detection_coverage_pct`,[siteId]);
  const correlationSelect = `ARRAY_REMOVE(ARRAY_AGG(DISTINCT NULLIF(gtm_tag_name,'')),NULL) AS gtm_tag_names,
  ...
  const eventsQ = vendor ? `SELECT ${displayName} AS event_name,event_type,vendor,
    ...
    COUNT(DISTINCT ${occurrenceKey})::int AS cnt,COUNT(DISTINCT session_id)::int AS sessions,
    ...
    COUNT(*) FILTER(WHERE ${networkObservation} AND delivery_outcome IN ('network_error','aborted','timeout','unknown'))::int AS transport_anomalies,
    ...
    FROM events WHERE site_id=$1 AND vendor=$2 AND received_at>NOW()-INTERVAL '24 hours' GROUP BY ${displayName},event_type,vendor ORDER BY cnt DESC LIMIT 100` : `SELECT ${displayName} AS event_name,event_type,vendor,
    MAX(NULLIF(params->>'conversion_label','')) AS conversion_label,
    MAX(COALESCE(NULLIF(params->>'conversion_id',''),NULLIF(params->>'google_conversion_id',''))) AS conversion_id,
    MAX(${platformId}) AS platform_id,${correlationSelect}
    COUNT(DISTINCT ${occurrenceKey})::int AS cnt,COUNT(DISTINCT session_id)::int AS sessions,
    ...
    COUNT(*) FILTER(WHERE ${networkObservation} AND delivery_outcome IN('network_error','aborted','timeout','unknown'))::int AS transport_anomalies,
    COUNT(*) FILTER(WHERE ${networkObservation} AND delivery_outcome='beacon_rejected')::int AS beacon_rejections,
    COUNT(*) FILTER(WHERE ${networkObservation} AND delivery_outcome='blocked')::int AS blocked,0 AS err
    FROM events WHERE site_id=$1 AND received_at>NOW()-INTERVAL '24 hours' GROUP BY ${displayName},event_type,vendor ORDER BY cnt DESC LIMIT 100`;
  const eventsRes = vendor ? await query(eventsQ,[siteId,vendor]) : await query(eventsQ,[siteId]);
  const flow = await query(`SELECT CASE WHEN delivery_mode IN('server_side','first_party') THEN 'first_party' WHEN delivery_mode IN('client_side','third_party') THEN 'third_party' ELSE 'unknown' END AS delivery_mode,
    COUNT(DISTINCT ${occurrenceKey})::int AS events,COUNT(DISTINCT COALESCE(NULLIF(session_id,''),NULLIF(client_id,'')))::int AS sessions,
    COUNT(*) FILTER(WHERE ${failedDelivery})::int AS failures,COUNT(DISTINCT resource_domain)::int AS destinations,ARRAY_REMOVE(ARRAY_AGG(DISTINCT resource_domain),NULL) AS domains
    FROM events WHERE site_id=$1 AND received_at>NOW()-INTERVAL '24 hours'
    GROUP BY CASE WHEN delivery_mode IN('server_side','first_party') THEN 'first_party' WHEN delivery_mode IN('client_side','third_party') THEN 'third_party' ELSE 'unknown' END ORDER BY events DESC`,[siteId]);
  const blockedFlow = await query(`SELECT CASE WHEN delivery_mode IN('server_side','first_party') THEN 'first_party' WHEN delivery_mode IN('client_side','third_party') THEN 'third_party' ELSE 'unknown' END AS delivery_mode,COUNT(*)::int AS blocked FROM adblock_events WHERE site_id=$1 AND confidence IN('confirmed','likely') AND ${noiseFilter} AND detected_at>NOW()-INTERVAL '24 hours' GROUP BY CASE WHEN delivery_mode IN('server_side','first_party') THEN 'first_party' WHEN delivery_mode IN('client_side','third_party') THEN 'third_party' ELSE 'unknown' END`,[siteId]);
