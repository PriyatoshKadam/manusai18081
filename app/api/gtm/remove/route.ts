...
function validId(value: unknown) { return typeof value === 'string' && /^[A-Za-z0-9_-]{1,80}$/.test(value); }
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const workspace = await gtmRequest<{workspaceId?:string;name?:string;tagManagerUrl?:string}>(`${base}/workspaces`,token,{method:'POST',body:JSON.stringify({name:`GAfix removal – ${installation.domain} – ${new Date().toISOString().slice(0,10)}`,description:'Created by GAfix to remove its managed monitor tag and publish the removal.'})});
    ...
    try{
      const tagsResponse=await gtmRequest<{tag?:Array<{tagId?:string;name?:string;firingTriggerId?:string[]}>}>(`${parent}/tags`,token);
      ...
      const versionResponse=await gtmRequest<{containerVersion?:{versionId?:string;name?:string;fingerprint?:string};compilerError?:boolean}>(`${parent}:create_version`,token,{method:'POST',body:JSON.stringify({name:`GAfix removal – ${installation.domain} – ${new Date().toISOString().replace('T',' ').slice(0,16)} UTC`,notes:'Created and published by GAfix after the user confirmed removal of the monitor.'})});
