...
// Platform reference catalog. Unknown parameters are intentionally allowed: a parameter is only
// invalid when the platform/spec explicitly says it is forbidden or when a known type contract is violated.
const CATALOG: Record<string, Record<string, ParameterSpec>> = {
  ga4: {
    ...
    cu:{status:'conditional',type:'string'}, pi:{status:'optional'}, pn:{status:'optional'}, cn:{status:'optional'}, cs:{status:'optional'},
    cm:{status:'conditional'}, cc:{status:'conditional'}, ct:{status:'conditional'}, ccf:{status:'conditional'}, cmt:{status:'conditional'},
  ...
  gads: {
    tid:{status:'required',type:'string'}, 'aw-id':{status:'required',type:'string'}, label:{status:'required',type:'string'}, l:{status:'required',type:'string'}, value:{status:'optional',type:'number'}, currency:{status:'optional',type:'string'}, oid:{status:'optional',type:'string'}, gclid:{status:'conditional'}, gclaw:{status:'auto'}, gcldc:{status:'auto'}, gclsrc:{status:'conditional'}, auid:{status:'undocumented'}, gcs:{status:'conditional'}, gcd:{status:'conditional'},
    email:{status:'optional'}, phone_number:{status:'optional'}, 'address.first_name':{status:'optional'}, 'address.last_name':{status:'optional'}, 'address.street':{status:'optional'}, 'address.city':{status:'optional'}, 'address.region':{status:'optional'}, 'address.postal_code':{status:'optional'}, 'address.country':{status:'optional'}, ec_mode:{status:'conditional'},
    ecomm_prodid:{status:'optional'}, ecomm_pagetype:{status:'optional'}, ecomm_totalvalue:{status:'optional',type:'number'}, items:{status:'optional',type:'array'},
  },
  meta: {
  ...
  tiktok: {
    event:{status:'required',type:'string'}, event_type:{status:'required',type:'string'}, event_id:{status:'required',type:'string'}, message_id:{status:'required',type:'string'}, event_time:{status:'required',type:'number'}, event_source:{status:'required',type:'string'}, event_source_url:{status:'required',type:'string'}, pixel:{status:'required'}, sdkid:{status:'required'},
    email:{status:'optional'}, phone_number:{status:'optional'}, external_id:{status:'optional'}, ttclid:{status:'conditional'}, ttp:{status:'auto'}, partner_id:{status:'optional'}, content_type:{status:'optional'}, content_id:{status:'optional'}, contents:{status:'optional',type:'array'}, content_name:{status:'optional'}, content_category:{status:'optional'}, currency:{status:'optional'}, value:{status:'optional',type:'number'}, quantity:{status:'optional',type:'number'}, description:{status:'optional'}, query:{status:'optional'}, status:{status:'optional'}, order_id:{status:'optional'}, shop_id:{status:'optional'},
  },
  linkedin: {
    pid:{status:'required',type:'string'}, fmt:{status:'auto'}, url:{status:'auto'}, time:{status:'auto'}, conversionId:{status:'conditional'}, li_fat_id:{status:'conditional'}, li_sugr:{status:'auto'}, cpuid:{status:'auto'},
    conversion:{status:'required',type:'string'}, conversionHappenedAt:{status:'required',type:'number'}, eventId:{status:'required',type:'string'}, 'user.userIds[]':{status:'required',type:'array'}, userIds:{status:'required',type:'array'}, 'user.userInfo':{status:'optional'}, 'user.lead':{status:'optional'}, 'user.externalIds[]':{status:'optional',type:'array'}, externalIds:{status:'optional',type:'array'}, 'conversionValue.currencyCode':{status:'optional'}, 'conversionValue.amount':{status:'optional'},
  },
  bing: {
  ...
  snapchat: {
    // Snapchat is supported by the dashboard, but the supplied reference contains no Snapchat section.
    // Do not invent required/optional rules; observed Snapchat parameters remain allowed unless GTM explicitly forbids them.
  },
};

const GA4_EVENT_SPECS: Record<string, Record<string, { type:string; presence:'required'|'optional' }>> = {
  ...
  view_item_list:{items:{type:'array',presence:'required'}}, begin_checkout:{items:{type:'array',presence:'required'},value:{type:'number',presence:'optional'},currency:{type:'string',presence:'optional'}},
  remove_from_cart:{items:{type:'array',presence:'required'}}, select_item:{items:{type:'array',presence:'required'}}, login:{method:{type:'string',presence:'optional'}}, sign_up:{method:{type:'string',presence:'optional'}}, generate_lead:{value:{type:'number',presence:'optional'},currency:{type:'string',presence:'optional'}},
};
...
export async function GET(req: NextRequest) {
  ...
  const observed = await query(`SELECT LOWER(COALESCE(event_name,'')) AS event_name, LOWER(parameter_name) AS parameter_name,
    ...
    GROUP BY LOWER(COALESCE(event_name,'')),LOWER(parameter_name) ORDER BY hits DESC`,args);

  const missing = await query(`SELECT LOWER(COALESCE(event_name,'')) AS event_name, LOWER(TRIM(value)) AS parameter_name,
      COUNT(DISTINCT COALESCE(NULLIF(session_id || ':' || occurrence_id, ':'), network_occurrence_id, id::text))::int AS missing_hits
  ...
  const snapshot = snapshotRows.rows[0] || null; const gtmSpecs = snapshot ? collectExplicitSpecs({tags:snapshot.tags,triggers:snapshot.triggers}) : [];
  const specByEventParam = new Map(gtmSpecs.map((s:any)=>[`${s.event}|${s.name}`,s]));

  const rows = new Map<string,any>();
  const ensure = (key:string) => { const row=rows.get(key)||{parameter_name:key,hits:0,hits_24h:0,hits_7d:0,events:new Set<string>(),sources:new Set<string>(),observed_types:new Set<string>(),missing_hits:0,specs:[]}; rows.set(key,row); return row; };
  for (const r of observed.rows) {
    const key=String(r.parameter_name); const row=ensure(key); row.hits+=Number(r.hits||0); row.hits_24h+=Number(r.hits_24h||0); row.hits_7d+=Number(r.hits_7d||0); if(r.event_name) row.events.add(r.event_name); asArray(r.sources).forEach(x=>x&&row.sources.add(String(x))); asArray(r.observed_types).forEach(x=>x&&row.observed_types.add(String(x))); const spec=specByEventParam.get(`${r.event_name}|${key}`); if(spec) row.specs.push(spec);
  }
  for (const r of missing.rows) { const key=String(r.parameter_name); const row=ensure(key); row.missing_hits+=Number(r.missing_hits||0); if(r.event_name) row.events.add(r.event_name); const spec=specByEventParam.get(`${r.event_name}|${key}`); if(spec&&!row.specs.some((s:any)=>s.event===spec.event&&s.name===spec.name)) row.specs.push(spec); }

  const catalog = CATALOG[vendor] || {};
  for (const [name,spec] of Object.entries(catalog)) {
    // Only surface a catalog parameter when observed or when it is a required wire/event parameter.
    if (spec.status==='required' && !rows.has(name)) ensure(name);
  }

  const output = Array.from(rows.values()).map(row => {
    const events=Array.from(row.events) as string[]; const observedTypes=Array.from(row.observed_types) as string[];
    const explicit=row.specs.find((s:any)=>s.type||s.presence||s.enum?.length||s.regex||s.conditional);
    const standard = vendor==='ga4' ? events.map(e=>GA4_EVENT_SPECS[e]?.[row.parameter_name]).find(Boolean) : undefined;
    const platform = catalog[row.parameter_name];
    const expectedType=explicit?.type || standard?.type || platform?.type || null;
    const presence=explicit?.presence || standard?.presence || platform?.status || (row.missing_hits>0?'required':'optional');
    const issues:any[]=[];

    // Unknown/optional/auto/conditional/undocumented parameters are NOT errors. They are valid traffic unless a GTM spec forbids them.
    if (expectedType && observedTypes.some(t=>t!=='null' && t!==expectedType)) issues.push({code:'type_collision',severity:'warning',title:'Type collision',message:`Expected ${expectedType}; observed ${observedTypes.join(', ')}.`,detail:`Observed across ${row.hits.toLocaleString()} parameter hits.`});
    if ((presence==='required') && row.missing_hits>0) issues.push({code:'missing_property',severity:'warning',title:'Missing required parameter',message:`Required parameter missing in ${row.missing_hits.toLocaleString()} observed event occurrences.`,detail:`Affected events: ${events.join(', ') || 'unknown'}.`});
    if (explicit?.presence==='forbidden') issues.push({code:'forbidden_property',severity:'warning',title:'Forbidden parameter',message:'This parameter is explicitly forbidden by the configured GTM specification.'});
    ...
    const status=issues.some(i=>i.severity==='warning')?'Warn':row.hits===0 && row.missing_hits===0?'Off':row.hits_24h===0?'Off':'OK';
    return {parameter_name:row.parameter_name,status,hits:row.hits,hits_24h:row.hits_24h,hits_7d:row.hits_7d,missing_hits:row.missing_hits,coverage:row.hits+row.missing_hits?Math.round(row.hits/(row.hits+row.missing_hits)*1000)/10:100,events,sources:Array.from(row.sources),observed_types:observedTypes,expected_type:expectedType,presence,reference_status:platform?.status || explicit?.presence || standard?.presence || 'observed',issues,last_seen:row.last_seen||null};
  }).sort((a,b)=>Number(b.hits||0)-Number(a.hits||0)||String(a.parameter_name).localeCompare(String(b.parameter_name)));
