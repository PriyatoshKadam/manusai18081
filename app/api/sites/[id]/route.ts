...
export async function GET(_req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const result = await query(`SELECT id, domain, gtm_container_id, ga4_measurement_id, gads_conversion_id, meta_pixel_id, tiktok_pixel_id, linkedin_partner_id, first_party_domain, CASE WHEN slack_webhook_url IS NOT NULL AND slack_webhook_url <> '' THEN true ELSE false END AS site_slack_configured FROM sites WHERE id=$1 AND user_id=$2`, [siteId, session.uid]);
