...
export async function GET(req: NextRequest) {
  ...
  const rows = health.rows.map((row: any) => ({ ...row, success_rate: Number(row.fires) ? Number(row.successes) / Number(row.fires) : 1, health_score: Math.max(0, Math.round((Number(row.successes) / Math.max(1, Number(row.fires))) * 100 - Math.min(30, Number(row.failures) * 2))) }));
  return NextResponse.json({ health: rows, anomalies: anomalies.rows, revenue: revenue.rows, compliance: compliance.rows, performance: performance.rows });
}
