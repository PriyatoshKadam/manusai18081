...
const successfulDelivery = `${networkObservation} AND status_code BETWEEN 200 AND 399 AND failure_reason IS NULL`;
...
export async function GET(req: NextRequest) {
  ...
  const rows = health.rows.map((row: any) => {
    ...
    const successRate = deliveryAttempts >= MIN_SAMPLE_SIZE ? (Number(row.successes) / deliveryAttempts) : null;
    const healthScore = deliveryAttempts >= MIN_SAMPLE_SIZE ? Math.max(0, Math.round((Number(row.successes) / Math.max(1, deliveryAttempts)) * 100 - Math.min(30, Number(row.failures) * 2))) : null;
    return { ...row, success_rate: successRate, health_score: healthScore, sample_size: deliveryAttempts, min_sample_size: MIN_SAMPLE_SIZE, sample_basis: 'network_delivery_attempts' };
  });
