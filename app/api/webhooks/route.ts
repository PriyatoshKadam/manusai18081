...
export async function POST(req: NextRequest) {
  ...
  try { const body = await req.json(); const siteId = Number(body?.siteId); const url = String(body?.url || '').trim(); if (!Number.isSafeInteger(siteId) || siteId <= 0 || !(await owns(siteId, session.uid))) return NextResponse.json({ error: 'Not found' }, { status: 404 }); if (!/^https:\/\//i.test(url)) return NextResponse.json({ error: 'Webhook URL must use HTTPS' }, { status: 400 }); const secret = String(body?.secret || '').trim(); const result = await query(`INSERT INTO site_webhooks (site_id, url, secret_encrypted, event_types) VALUES ($1,$2,$3,$4::jsonb) ON CONFLICT (site_id,url) DO UPDATE SET secret_encrypted=COALESCE(EXCLUDED.secret_encrypted,site_webhooks.secret_encrypted), event_types=EXCLUDED.event_types, enabled=true RETURNING id,url,event_types,enabled,created_at`, [siteId, url, secret ? encryptSecret(secret) : null, JSON.stringify(Array.isArray(body?.eventTypes) ? body.eventTypes.slice(0, 20) : ['alert'])]); return NextResponse.json({ webhook: result.rows[0] }, { status: 201 }); } catch (error) { return NextResponse.json({ error: error instanceof Error ? error.message : 'Unable to save webhook' }, { status: 400 }); }
}
