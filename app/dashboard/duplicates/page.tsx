...
export default function DuplicatesPage() {
  ...
  return (
    <div className="fade-in max-w-5xl">
      ...
      {retries.length > 0 && <section className="bg-blue-50 border border-blue-200 rounded-xl p-5 mt-6"><h3 className="font-semibold text-blue-950">Transport retries — not duplicate fan-out</h3><p className="text-sm text-blue-900 mt-1">These requests share a normalized signature, but the first attempt failed and a later attempt succeeded within five seconds. They are shown for transport investigation and are excluded from duplicate counts.</p><div className="mt-4 space-y-2">{retries.slice(0, 20).map((retry: any) => <div key={retry.id} className="rounded-lg border border-blue-100 bg-white/70 p-3 text-sm"><div className="flex items-center justify-between gap-3"><span className="mono font-semibold text-blue-950">{retry.event_name || '(unnamed)'}</span><span className="text-xs text-blue-800">{Math.round(Number(retry.raw?.gapMs || 0))} ms gap</span></div><div className="mt-1 text-xs text-blue-900">Session: {retry.raw?.sessionId || 'not available'} · Failed evidence: {retry.raw?.failedEventId || '—'} · Successful evidence: {retry.raw?.successEventId || '—'}</div></div>)}</div></section>}
    </div>
  );
}
