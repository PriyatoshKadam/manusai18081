  ...
  return (
    <section className="rounded-2xl border border-white/[.08] bg-[#111722] overflow-hidden">
      <div className="flex items-start justify-between gap-4 border-b border-white/[.06] px-5 py-4">
        <div>
          <h3 className="font-semibold text-slate-100">Events → sessions</h3>
          <p className="mt-1 text-xs text-slate-400">High volume with low session spread is a duplicate-risk signal.</p>
        </div>
      ...
      {rows.length === 0 ? <div className="p-6 text-sm text-slate-500">No event volume has arrived yet.</div> : (
            ...
            return (
              <div key={`${eventLabel(row)}-${index}`} className="grid grid-cols-[minmax(110px,160px)_1fr_72px] items-center gap-3 text-xs">
                <div className="min-w-0">
                  ...
                  <div className="mt-1 text-[10px] text-slate-500">{ratio ? `${ratio.toFixed(1)}× per session` : 'No session id'}</div>
                </div>
  ...
  return (
    <section className="rounded-2xl border border-white/[.08] bg-[#111722] overflow-hidden">
      <div className="border-b border-white/[.06] px-5 py-4">
        <h3 className="font-semibold text-slate-100">How events entered the system</h3>
        <p className="mt-1 text-xs text-slate-400">DataLayer, network, GTM, and direct-code evidence for the same event names.</p>
      </div>
      {rows.length === 0 ? <div className="p-6 text-sm text-slate-500">No source evidence yet.</div> : <div className="space-y-3 p-5">{rows.map((row, index) => <div key={`${row.event_name}-${row.source}-${index}`}><div className="mb-1 flex justify-between gap-3 text-xs"><span className="truncate mono text-slate-300">{row.event_name || '(unnamed)'} <span className="text-slate-500">· {row.source || row.observation_kind || 'unknown'}</span></span><span className="font-mono text-slate-400">{formatNumber(row.count)}</span></div><div className="h-1.5 overflow-hidden rounded-full bg-white/[.06]"><div className={`h-full rounded-full ${row.observation_kind === 'datalayer' ? 'bg-violet-500' : row.observation_kind === 'network' ? 'bg-cyan-500' : 'bg-amber-500'}`} style={{ width: `${Math.max(5, (Number(row.count || 0) / max) * 100)}%` }} /></div></div>)}</div>}
    </section>
  );
}
...
export function DataLayerProvenance({ rows }: { rows: any[] }) {
  ...
  return (
    <section className="rounded-2xl border border-white/[.08] bg-[#111722] overflow-hidden">
      <div className="flex items-start justify-between gap-4 border-b border-white/[.06] px-5 py-4">
        <div>
          <h3 className="font-semibold text-slate-100">Event provenance</h3>
          <p className="mt-1 text-xs text-slate-400">Every row answers: which event, which push, which session, and which observation path?</p>
        </div>
        <span className="pill bg-violet-500/10 text-[#c4acff]">{formatNumber(rows.length)} recent rows</span>
      </div>
      {latest.length === 0 ? <div className="p-6 text-sm text-slate-500">No dataLayer provenance has been recorded yet.</div> : <div className="divide-y divide-white/[.06]">{latest.map((row, index) => <div key={`${row.event_name}-${row.received_at}-${index}`} className="grid gap-3 px-5 py-3 md:grid-cols-[minmax(140px,1.2fr)_100px_130px_minmax(150px,1fr)] md:items-center"><div className="min-w-0"><div className="truncate font-medium text-slate-200 mono">{row.event_name || '(unnamed)'}</div><div className="mt-1 truncate text-[10px] text-slate-500">{row.page_url || 'page unavailable'}</div></div><div><div className="text-[10px] uppercase tracking-wider text-slate-500">Push</div><div className="mt-1 font-mono text-sm text-[#c4acff]">{row.dl_push_index === null || row.dl_push_index === undefined ? '—' : `#${row.dl_push_index}`}</div></div><div><div className="text-[10px] uppercase tracking-wider text-slate-500">Session</div><div className="mt-1 font-mono text-xs text-slate-400">{shortSession(row.session_id)}</div></div><div className="flex flex-wrap items-center gap-2"><span className={`pill ${row.observation_kind === 'datalayer' ? 'bg-violet-500/10 text-[#c4acff]' : row.observation_kind === 'network' ? 'bg-cyan-500/10 text-[#7de2ff]' : 'bg-amber-500/10 text-[#ffd27a]'}`}>{row.observation_kind || 'unknown'}</span><span className="pill bg-white/[.06] text-slate-400">{row.source || 'unknown source'}</span>{row.occurrence_id ? <span className="text-[10px] text-slate-500 mono">{String(row.occurrence_id).slice(0, 18)}</span> : null}</div></div>)}</div>}
    </section>
  );
}
...
export function DuplicateEvidenceChart({ duplicates }: { duplicates: any[] }) {
  ...
  return (
    <section className="mb-6 rounded-2xl border border-white/[.08] bg-[#111722] overflow-hidden">
      <div className="flex items-start justify-between gap-4 border-b border-white/[.06] px-5 py-4">
        <div>
          <h3 className="font-semibold text-slate-100">Duplicate pressure by event</h3>
          <p className="mt-1 text-xs text-slate-400">Purple marks same-push GTM fan-out; amber marks repeated event or network evidence.</p>
        </div>
        <span className="pill bg-rose-500/10 text-[#ff9aae]">{formatNumber(duplicates.length)} evidence records</span>
      </div>
      {grouped.length === 0 ? <div className="p-6 text-sm text-slate-500">No duplicate evidence to chart.</div> : <div className="space-y-4 p-5">{grouped.map((row) => <div key={row.name} className="grid grid-cols-[130px_1fr_60px] items-center gap-3 text-xs"><div className="truncate mono font-medium text-slate-200">{row.name}</div><div className="space-y-1"><div className="flex h-2 overflow-hidden rounded-full bg-white/[.06]"><div className="bg-violet-500" style={{ width: `${Math.max(4, (row.fanout / row.count) * (row.count / max) * 100)}%` }} /><div className="bg-amber-400" style={{ width: `${Math.max(4, (row.repeat / row.count) * (row.count / max) * 100)}%` }} /></div><div className="flex gap-3 text-[10px] text-slate-500"><span>{row.fanout} fan-out</span><span>{row.repeat} repeat</span></div></div><div className="text-right font-mono font-medium text-slate-400">{row.count}</div></div>)}</div>}
    </section>
  );
}
