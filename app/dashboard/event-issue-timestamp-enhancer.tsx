...
type GovernanceIssue = { code?: string; message?: string; detail?: string; timestamp?: string | null };
type GovernanceRow = { event_name?: string; last_seen?: string | null; issues?: GovernanceIssue[] };
  ...
  useEffect(() => {
    ...
    const render = async () => {
      try {
        ...
        const governance: GovernanceRow[] = Array.isArray(payload?.events) ? payload.events : [];
        const byName = new Map<string, GovernanceRow>();
        governance.forEach((row) => byName.set(String(row.event_name || '').trim().toLowerCase(), row));
        ...
        // Keep the existing table intact if governance data cannot be loaded.
      }
    };
