...
export default function FlowSummaryGraph({ rows = [], blockedRows = [], sample = false, compact = false }: FlowSummaryGraphProps) {
  ...
  return <section className={`overflow-hidden rounded-[24px] border border-white/[.08] bg-[#111722] shadow-2xl shadow-black/20 ${compact ? '' : 'shadow-black/20'}`}>
    <div className="flex flex-col gap-3 border-b border-white/[.08] px-5 py-5 sm:flex-row sm:items-center sm:justify-between sm:px-7">
      <div><div className="text-[10px] font-bold uppercase tracking-[.18em] text-[#b8f56b]">Delivery path intelligence</div><h3 className="mt-1.5 text-xl font-semibold tracking-tight text-white">Where did the event travel?</h3><p className="mt-1.5 max-w-2xl text-sm leading-6 text-slate-500">Client-side calls go straight from the browser to a platform host. Server-side calls first reach your own or configured custom domain, which can remain reachable when platform hosts are blocked.</p></div><span className="w-fit rounded-full border border-[#a8f06a]/20 bg-[#a8f06a]/10 px-3 py-1.5 text-xs font-semibold text-[#cfff9d]">24h evidence window</span>
    </div>
    <div className="p-4 sm:p-7">
      ...
      <div className="mt-5 grid gap-4 lg:grid-cols-2">
        ...
        <PathCard title="Server-side path" tone="lime" row={server} blocked={serverBlocked} domains={serverDomains} description="The browser reaches your first-party or custom endpoint first. GAfix labels this path separately so an ad-blocked client call is not treated as a failed server-side event." />
      </div>
      {unknown.events || !hasEvidence ? <div className="mt-4 rounded-xl border border-dashed border-white/10 bg-white/[.04] px-4 py-3 text-xs leading-5 text-slate-400">{hasEvidence ? `${number(unknown.events)} event(s) have an unclassified destination. Configure the site's first-party/custom domain or inspect the raw request host before treating it as client-side or server-side.` : 'Waiting for network evidence. The graph will classify requests from their observed destination host; dataLayer-only observations remain unclassified until a network request is correlated.'}</div> : null}
      {sample ? <div className="mt-6 border-t border-white/[.08] pt-5"><div className="mb-3 flex items-center justify-between"><div><p className="text-[10px] font-bold uppercase tracking-[.16em] text-[#b8f56b]">Sample event rail</p><p className="mt-1 text-sm font-semibold text-white">Same business events, two delivery paths</p></div><span className="font-mono text-[10px] text-slate-400">LIVE-LIKE SAMPLE</span></div><div className="space-y-2">{sampleEvents.map((event) => <div key={`${event.time}-${event.name}-${event.path}`} className="grid grid-cols-[58px_1fr_auto] items-center gap-3 rounded-xl bg-white/[.04] px-3 py-2.5 text-xs sm:grid-cols-[72px_1fr_130px_72px_70px]"><span className="font-mono text-[10px] text-slate-400">{event.time}</span><span className="min-w-0 truncate font-semibold text-white">{event.name}<span className="ml-2 font-normal text-slate-400">{event.vendor}</span></span><span className={`hidden rounded-full px-2 py-1 text-center text-[10px] font-bold sm:block ${event.path === 'server_side' ? 'bg-[#a8f06a]/10 text-[#cfff9d]' : 'bg-[#f6b94c]/10 text-[#ffd27a]'}`}>{event.path === 'server_side' ? 'server-side' : 'client-side'}</span><span className="hidden text-right font-mono text-[10px] text-slate-500 sm:block">{event.status}</span><span className="text-right font-mono text-[10px] text-slate-400">{event.latency}</span></div>)}</div></div> : null}
    </div>
  </section>;
}
