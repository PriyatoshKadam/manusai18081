...
export default function GtmConnectPage() {
  ...
  async function refreshInventory() {
    ...
    try {
      ...
      if (data.detectedInstallation) {
        setInstallation(data.detectedInstallation);
        setNotice('GAfix monitor script detected in the scanned workspace. You can remove the script, then publish the removal separately.');
      } else {
        setNotice('Tag Manager setup details refreshed. No published GAfix monitor script was detected in this workspace.');
      }
  ...
  async function removeMonitor() {
    if (!installation || installation.status !== 'published') return;
      const confirmed = window.confirm('Prepare removal of the GAfix monitor script from the live Tag Manager container? This will create a removal workspace, but will not publish until you confirm the separate publish action.');
    if (!confirmed) return;
    setLoading(true); setError(null); setNotice(null);
    try {
      ...
      setInstallation((current) => current ? { ...current, status: 'removal_ready', publishRequired: true } : current); setNotice('The GAfix monitor removal is prepared in a new GTM workspace. Publish the removal separately when you are ready.');
