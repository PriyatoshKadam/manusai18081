...
export default function HealthPage() {
  ...
  return <div className="fade-in mx-auto max-w-[1500px] space-y-7">
    <div className="dashboard-section-head"><div><div className="dashboard-eyebrow">Tracking health · Last 24 hours</div><h2>How your tracking is doing</h2><p>Every score is based on what visitors experienced: tracking activity, problems, response time, privacy choices, and unusual changes.</p></div><div className="dashboard-top-control"><span className="dot bg-[#a8f06a]" /><strong>Updating automatically</strong> · 15s</div></div>
    ...
    <section className="grid gap-4 lg:grid-cols-[1.2fr_1fr_1fr_1fr]">
      <div className="relative overflow-hidden rounded-2xl border border-white/[.08] bg-[#111722] p-5"><div className="absolute -right-10 -top-10 h-32 w-32 rounded-full bg-[#a8f06a]/10 blur-2xl" /><div className="relative flex items-center gap-5"><ScoreRing value={avgHealth} label="Overall health" detail={health.length ? `${health.length} tracked signals` : 'Collecting'} /><div><div className="text-[10px] uppercase tracking-[.16em] text-slate-500">Quick summary</div><div className="mt-2 text-lg font-semibold text-white">{avgHealth === null ? 'Waiting for visitor activity' : avgHealth >= 95 ? 'Tracking looks healthy' : avgHealth >= 80 ? 'Worth checking' : 'Needs attention'}</div><p className="mt-2 max-w-[220px] text-xs leading-5 text-slate-400">This combines tracking success and website speed using real visitor data.</p></div></div></div>
      <CommandKpi label="Tracking actions seen" value={format(totalFires)} note={`${health.length} different tracking actions`} tone="blue" />
      <CommandKpi label="Actions with problems" value={format(totalFailures)} note={totalFires ? `${Math.round((totalFailures / totalFires) * 1000) / 10}of tracking actions` : 'No problems found'} tone={totalFailures ? 'rose' : 'lime'} />
