...
export default function DashboardPage() {
  ...
  useEffect(() => { if (!siteId) return; let cancelled = false; setLoading(true); Promise.all(platforms.map(async (platform) => { const response = await fetch(`/api/platform-insights?siteId=${siteId}&vendor=${platform.vendor}`, { cache: 'no-store' }); if (!response.ok) return [platform.vendor, null] as const; const data = await response.json(); return [platform.vendor, data.overview as Summary] as const; })).then((rows) => { if (!cancelled) setSummary(Object.fromEntries(rows.filter((row): row is readonly [string, Summary] => Boolean(row[1])))); }).finally(() => { if (!cancelled) setLoading(false); }); return () => { cancelled = true; }; }, [siteId]);
