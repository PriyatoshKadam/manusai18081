...
function activeTab(label:string){const b=findTabButton(label);if(!b)return false;return b.getAttribute('aria-selected')==='true'||b.getAttribute('data-state')==='active'||getComputedStyle(b).backgroundColor!=='rgba(0, 0, 0, 0)'&&getComputedStyle(b).backgroundColor!=='transparent';}
...
export default function PlatformEvidenceSemanticsEnhancer({vendor}:{vendor:string}){
 useEffect(()=>{
  ...
  const observer=new MutationObserver(()=>{if(timer)clearTimeout(timer);timer=setTimeout(()=>void render(),100)});observer.observe(document.body,{childList:true,subtree:true});void render();return()=>{cancelled=true;if(timer)clearTimeout(timer);observer.disconnect();removeOwn()};
 },[vendor]);
 return null;
}
