...
function icon(kind: 'dashboard' | 'revenue' | 'alerts' | 'gtm' | 'settings') {
  const paths = {
...
export default function DashboardShell({ children, email, sites }: { children: React.ReactNode; email: string; sites: Site[] }) {
  ...
  const nav = [
    ...
    { href: '/dashboard/revenue', label: 'Revenue', icon: 'revenue' as const, active: pathname.startsWith('/dashboard/revenue') },
...
function pageTitle(path: string) { if (path === '/dashboard') return 'Dashboard'; if (path.startsWith('/dashboard/revenue')) return 'Revenue'; if (path === '/dashboard/alerts') return 'Alerts'; if (path.startsWith('/dashboard/gtm')) return 'GTM Diagnostic'; if (path.startsWith('/dashboard/settings')) return 'Settings'; const platform = platforms.find((item) => item.href === path); return platform?.label || 'Dashboard'; }
