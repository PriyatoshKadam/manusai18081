...
function noteForEvent(event: any, alerts: any[]) {
  const related = alerts.filter((alert) => alert.event_name === event.event_name);
  if (Number(event.failed || 0) > 0) return `${fmt(event.failed)} failed delivery${Number(event.failed) === 1 ? '' : 'ies'}.`;
  if (Number(event.blocked || 0) > 0) return `${fmt(event.blocked)} delivery${Number(event.blocked) === 1 ? '' : 'ies'} blocked.`;
  if (Number(event.transport_anomalies || 0) > 0) return `${fmt(event.transport_anomalies)} transport anomal${Number(event.transport_anomalies) === 1 ? 'y' : 'ies'}.`;
...
export default function VendorView({ vendor, label, id }: { vendor: string; label: string; id: string | null }) {
  ...
  const trendsByEvent = useMemo(() => {
    ...
    for (const row of insights.eventTrends || []) {
      const current = map.get(row.event_name) || [];
      current.push(Number(row.count || 0));
  ...
  return <div className="fade-in mx-auto max-w-[1500px] space-y-5">
    <header className="flex flex-col gap-3 md:flex-row md:items-end md:justify-between">
      <div><p className="dashboard-eyebrow">Platform monitoring</p><div className="mt-1 flex items-center gap-3"><span className="grid h-10 w-10 place-items-center rounded-xl bg-[var(--mon-tint)] text-xs font-bold text-[var(--mon-fg)]">{activeVendor.slice(0, 2).toUpperCase()}</span><div><h2 className="font-display text-2xl font-semibold tracking-tight text-[var(--text)]">{activeVendor}</h2><p className="text-xs text-[var(--text-3)]">{id ? `Destination ${id}` : 'Destination configuration not detected'} · last 24 hours</p></div></div></div>
      <Link href={`/dashboard/alerts?siteId=${siteId}`} className="rounded-xl border border-[var(--border)] bg-[var(--surface)] px-4 py-2.5 text-xs font-semibold text-[var(--text)] hover:border-[var(--border-strong)]">View alerts →</Link>
    </header>
    ...
    {tab === 'overview' && <section className="space-y-4">
      <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-6"><Kpi label="Total event hits" value={fmt(totalEventHits)}/><Kpi label="Event total" value={fmt(overview.event_total || events.length)}/><Kpi label="Parameters number" value={fmt(overview.parameter_number || parameters.length)}/><Kpi label="New issues" value={fmt(newIssues)} tone="warn"/><Kpi label="Still open" value={fmt(stillOpen)} tone={stillOpen ? 'warn' : 'ok'}/><Kpi label="New event type detected" value={fmt(newEventTypes)} tone={newEventTypes ? 'info' : 'ok'}/></div>
      <div className="grid gap-4 xl:grid-cols-[1.3fr_.7fr]">
        <Panel title="Platform health" subtitle="Delivery and detection evidence from the last 24 hours"><div className="grid gap-3 sm:grid-cols-3"><HealthMetric label="Successful network events" value={fmt(overview.successful_network_events)}/><HealthMetric label="Failed network events" value={fmt(overview.failed_network_events)} warn={Number(overview.failed_network_events)>0}/><HealthMetric label="Open alerts" value={fmt(stillOpen)} warn={stillOpen>0}/></div></Panel>
        <Panel title="What needs attention" subtitle="Issues are ranked from the existing detection evidence.">{alerts.slice(0, 5).map((alert: any, index: number) => <div key={`${alert.id || index}`} className="flex gap-3 border-b border-[var(--border-soft)] py-3 last:border-0"><span className="mt-1 h-2 w-2 rounded-full" style={{background:alert.severity==='critical'?'#dc2626':'#f59e0b'}}/><div className="min-w-0"><div className="truncate text-xs font-semibold text-[var(--text)]">{alert.event_name || alert.code || 'Tracking issue'}</div><div className="mt-0.5 text-[11px] text-[var(--text-2)]">{alert.message || alert.code || 'Review this alert.'}</div></div></div>)}{!alerts.length?<Empty text="No open platform alerts."/>:null}</Panel>
      </div>
    </section>}
...
function Table({ children }: { children: React.ReactNode }) { return <div className="overflow-x-auto rounded-2xl border border-[var(--border)] bg-[var(--surface)] shadow-[var(--shadow)]"><table className="w-full min-w-[900px] border-collapse text-left"><tbody>{children}</tbody></table></div>; }
