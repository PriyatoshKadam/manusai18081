...
export async function recordComplianceEvidence(event: ParsedEvent, site: { domain: string; firstPartyDomain?: string | null }) {
  ...
  if (!['script_injected', 'resource_error', 'resource_blocked', 'csp_violation'].includes(diagnostic)) return;
  ...
  const category = diagnostic === 'script_injected' && !known ? 'unknown_script' : diagnostic === 'csp_violation' ? 'csp_violation' : diagnostic === 'resource_error' ? 'resource_error' : 'supply_chain';
