...
async function createAlert(input:{siteId:number;severity:string;code:string;category?:string;vendor:string|null;eventName:string|null;message:string;rootCause:string;fixSteps:string[];pageUrl:string;raw:Record<string,unknown>;occurrenceCount?:number;distinctPushes?:number;dedupeMinutes?:number}){
  ...
  const existing=await query(`UPDATE alerts SET occurrence_count=COALESCE(occurrence_count,1)+1,last_seen=NOW(),raw=$6::jsonb WHERE site_id=$1 AND code=$3 AND COALESCE(vendor,'')=COALESCE($5,'') AND COALESCE(event_name,'')=COALESCE($4,'') AND resolved=false AND created_at>=NOW()-($7*INTERVAL '1 minute') RETURNING id`,[input.siteId,input.severity,input.code,input.eventName,input.vendor,JSON.stringify(input.raw),dedupeMinutes]); if(existing.rowCount)return;
