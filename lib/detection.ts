...
function isFailedTransport(event: Pick<ParsedEvent,'statusCode'|'failureReason'>){ return Boolean((event.statusCode !== null && event.statusCode !== undefined && event.statusCode >= 400) || event.failureReason); }
export function isTransportRetryPair(current: Pick<ParsedEvent,'requestSignature'|'statusCode'|'failureReason'|'receivedAt'>, previous: Pick<DuplicateMatch,'requestSignature'|'statusCode'|'failureReason'|'receivedAt'>){
  ...
  const currentSucceeded = !isFailedTransport(current);
